...
clang_prepath = 'prebuilts/clang/host/linux-x86/clang-4691093/'
clang_lib_path = project_path + clang_prepath + 'lib64/libc++.so'
# clang_lib_path = '/hci/chaoran_data/android-9.0.0_r47/out/host/linux-x86/lib64/libclang_android.so'
clang_lib_path = '/hci/chaoran_data/android-9.0.0_r47/prebuilts/clang/host/linux-x86/clang-4691093/lib64/libclang.so.6'
...
init_arg_config = '-isystem/hci/chaoran_data/android-9.0.0_r47/prebuilts/clang/host/linux-x86/clang-4691093/lib64/clang/6.0.2/include'
clang_bin_path = '/hci/chaoran_data/android-9.0.0_r47/prebuilts/clang/host/linux-x86/clang-4691093/bin'
h_list = None
compdb = False
if(compdb):
    file_json_key = 'file'
    ...
    def run(self, file_str, pro_path,  ninja_args, entry_funs=None, IS_AOSP=True, extend_analyze=True, show_loc=False, print_all_node=False, generate_html=False, anti_search=False, only_preprocess = False):
        ...
        if extend_analyze:
            ...
            for tem in not_sys_include_paths:
                ...
                # print("*.h:", tem)
                if IS_AOSP:
                    c_cpp_list = find_command(tem, compdb=False, project_path=project_path)
...
def test11():
    ...
    with open('jni/jni.json') as file_obj:
        ...
        for cpp_jni in cpp_jni_list:
            ...
            print(entry_funs)
            c_cpp_list = find_command_star_node(cpp.replace(project_path, ''), compdb=False)
            c_cpp_list = c_cpp_list[0]
