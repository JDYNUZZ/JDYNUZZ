...
def process_soong_ninja():
    ...
    # 加载配置项
    file = open('/Volumes/android/android-8.0.0_r34/out/soong/build.ninja', 'r')
    global_dic = {}
        ...
        if line:
            if not line.strip().startswith('#'):
                ...
                # build模式结束
                    ...
                    if ('clang' in rule_command_split[0] or 'clang++' in rule_command_split[0]):
        ...
        if line:
            ...
            if '#' not in line and line.startswith('command') and (line.endswith('.cpp"') or line.endswith('.c"')):
                ...
                if 'frameworks/native/opengl/libagl/egl.cpp' in source:
                    iiiii=0
                content = {'source': source, 'content': {'compiler': compiler, 'source': source, 'flags': flags}}
                json_list.append(content)
        else:
            break
    file.close()
