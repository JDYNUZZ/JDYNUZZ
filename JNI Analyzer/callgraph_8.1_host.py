...
clang_prepath = 'prebuilts/clang/host/linux-x86/clang-4053586/'
clang_lib_path = project_path + clang_prepath + 'lib64/libc++.so'
clang_lib_path = '/hci/chaoran_data/android-8.1.0_r67/out/host/linux-x86/lib64/libclang.so'
...
init_arg_config = '-isystem/hci/chaoran_data/android-8.1.0_r67/prebuilts/clang/host/linux-x86/clang-4053586/lib64/clang/5.0/include'
clang_bin_path = '/hci/chaoran_data/android-8.1.0_r67/prebuilts/clang/host/linux-x86/clang-4053586/bin'
h_list = None
compdb = False
if(compdb):
    file_json_key = 'file'
...
class file_analyser():
    ...
    def show_info(self, node, cur_fun=None, depth=0, print_node=False):
        ...
        if 'setMute' in node.displayname:
            print_node = True
    ...
    def load_cfg_normal(self, index, file, args):
        ...
        syspath = ccsyspath.system_include_paths(clang_bin_path)
    ...
    def load_cfg(self, index, compiler, file, ninja_args):
        ...
        ast_path = version + 'ast/' + file.replace('/','_') + '.ast'
        tu = None
        if compdb:
        ...
        # 去掉 '-Wno-error=unused-lambda-capture'
        if '-Wno-error=unused-lambda-capture' in ninja_args:
            ninja_args.remove('-Wno-error=unused-lambda-capture')
    ...
    def run(self, file_str, pro_path,  ninja_args, entry_funs=None, IS_AOSP=True, extend_analyze=True, show_loc=False, print_all_node=False, generate_html=False, anti_search=False, only_preprocess = False):
        ...
        if extend_analyze:
            ...
            for tem in not_sys_include_paths:
                ...
                # print("*.h:", tem)
                if IS_AOSP:
                    c_cpp_list = find_command(tem, version_prefix='8.1', compdb=False, project_path=project_path)
                    if c_cpp_list is not None:
                        next_file = project_path + c_cpp_list[file_json_key]
                        '''
...
def test9():
    c_cpp_list = find_command_star_node('frameworks/base/core/jni/android_hardware_Radio.cpp', compdb=False)
    ...
    main_file_analyser = file_analyser()
    # analyze cpp
    file = project_path + c_cpp_list[file_json_key]
...
def test11():
    start = False
    with open('jni8.1/jni.json') as file_obj:
        ...
        for cpp_jni in cpp_jni_list:
            ...
            for var in vars:
                for pair in var:
                    # print(pair[-1]+'(')
                    if 'android_media_AudioRecord_start' in pair[-1]+'(':
                        entry_funs.append(pair[-1]+'(')
            print(entry_funs)
            c_cpp_list = find_command_star_node(cpp.replace(project_path, ''), version_prefix='8.1', compdb=False)
            c_cpp_list = c_cpp_list[0]
