    ...
    def extract_onTransact(self, node, identifier, conditions, depth=0, case_mode=False, case_level = 10e10, clearId=False):
        if node is not None and node.location.file is not None and self.pro_path in node.location.file.name:
            for n in node.get_children():
                ...
                if n is not None:
                    self.extract_onTransact(n, identifier, conditions, case_mode=case_mode, case_level=case_level, depth=depth + 1, clearId=clearId)
                        ...
                        if self.has_ignore_fun_Ixx(method_full) and len(tema)>0:
                            ...
                            if 'ISurfaceComposer::captureScreen' in self.fully_qualified_pretty(f) and return_class !='no_caller':
                                ...
                                if Ixx_method_cpp_k:
                                    ...
                                    conditions = self.onTransact(transact, return_class, clearId=clearId)
                                    if clearId:
                                        print('=== 2129 ===')
                                        exit(0)
