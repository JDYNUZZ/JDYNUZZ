...
class file_analyser():
                ...
                if tem[0] > call_mode_last_level and not children_ignore and (tem[1].kind == CursorKind.MEMBER_REF_EXPR or tem[1].kind == CursorKind.DECL_REF_EXPR):
                    ...
                    if tem[1].displayname != call_node.displayname and 'operator->' not in tem[1].displayname and \
                        ...
                        if tem[1].displayname == var:
                            ...
                            print(call_node.kind, call_node.displayname, call_node.location)
                            print(call_mode_para_index)
    ...
    def _get_fun_con(self, cursor, debug=True):
        ...
        if cursor.location.file.name.endswith('.h'):
        ...
        if debug:
            print('|||||||||||cursor.referenced|||', cursor.kind, cursor.displayname, cursor.location)
        children = []
        ...
        for child in children:
            ...
            if node.kind == CursorKind.SWITCH_STMT:
                mode = 'switch'
                break
            if 'checkCallingPermission' in node.displayname:
                mode = 'permission'
                break
        if mode == 'switch':
            for child in children:
            ...
            for child in children:
                node = child[1]
                if debug:
                    print('|||||||permission||||', node.kind, node.displayname)
                if 'checkCallingPermission' in node.displayname and node.kind == CursorKind.CALL_EXPR:
    ...
    def _get_fun_in_condition(self, cursor, num=1, debug=True):
        children = []
        self.get_all_children(cursor, children, level=0)
        funs = []
        for child in children:
    ...
    def show_info(self, node, cur_fun=None, depth=0, print_node=False, if_stmt=None, last_node=None):
        ...
        #     print(fun_str)
        #     o=0
        # if 'MediaRecorderClient::MediaRecorderClient(' in fun_str:
        #     print(node.kind)
        #     print(fun_str)
        #     print(self.fully_qualified_pretty(node))
        ...
        if node.location.file and (self.print_all_node or print_node) and self.pro_path in node.location.file.name:
            print('\n%2d' % depth + ' ' * depth, node.kind, node.displayname, end='')
    ...
    def search_fun_list_full(self, fun_name):
        ...
        print('*** search for fun %s ***' % fun_name)
        k_list = []
        v_list = []
        for k, v in self.FULLNAMES.items():
            k4match = k
                ...
                if(f.location.file is not None and self.pro_path in f.location.file.name):
                    ...
                    if len(r_final) > 0 and self.has_no_ignore_fun(self.fully_qualified_pretty(last_node)) and self.has_no_ignore_fun(self.fully_qualified_pretty(f)):
                        ...
                        if last_node.location.file.name.endswith('.h'):
                            ...
                            for tem_v_list in v_list:
                                ...
                                if tem_v_list.location.file.name.endswith('.cpp'):
                                    print('last_node 替换为', tem_v_list.kind, self.fully_qualified_pretty(tem_v_list), tem_v_list.location.file.name)
                        ...
                        if 'android::hardware::' in method_full:
                            ...
                            continue
                        if 'checkPermission' in method_full:
                            print('*** END OF CALL because found checkPermission()')
                            continue
                        if 'getMediaPlayerService' in method_full:
    ...
    def run(self, file_str, pro_path,  ninja_args, entry_funs=None, IS_AOSP=True, extend_analyze=True, show_loc=False, print_all_node=False, generate_html=False, anti_search=False, only_preprocess = False):
            ...
            if IS_AOSP:
                if self.pro_path + 'frameworks' in include_path:
                    ...
                    if len(r) > 0 and 'Service' not in include_path:
                        ...
                        # not_sys_include_paths.append(include_path.replace(r[0], 'AudioFlinger.h'))
                        # not_sys_include_paths.append(include_path.replace(r[0], 'AudioPolicyService.h'))
                        # not_sys_include_paths.append(include_path.replace(r[0], 'ServiceUtilities.h'))
                        # not_sys_include_paths.append('frameworks/av/services/audiopolicy/service/AudioPolicyInterfaceImpl.cpp')

                        # not_sys_include_paths.append('/Volumes/android/android-8.0.0_r34/frameworks/av/services/audioflinger/ServiceUtilities.cpp')

                        # not_sys_include_paths.append('/Volumes/android/android-8.0.0_r34/frameworks/av/include/media/IMediaPlayerService.h')
                        # not_sys_include_paths.append('/Volumes/android/android-8.0.0_r34/frameworks/av/include/media/IMediaDeathNotifier.h')
        ...
        if extend_analyze:
            ...
            not_sys_include_paths.insert(0, '/Volumes/android/android-8.0.0_r34/frameworks/av/services/audioflinger/AudioFlinger.h')
            not_sys_include_paths.insert(0, '/Volumes/android/android-8.0.0_r34/frameworks/av/services/audiopolicy/service/AudioPolicyService.h')
            not_sys_include_paths.insert(0, '/Volumes/android/android-8.0.0_r34/frameworks/av/services/audioflinger/ServiceUtilities.h')
            ...
            for [method, permission] in self.found_permission_method:
                if isinstance(permission, list):
                    ...
                    if not permission.endswith('&&'):
                        print('FOUND ', method, ' ::: ', '[%s]' % permission_tem)
                        permission_tem = ''
                # print('FOUND ', method, ' ::: ', '[%s]' % permission[0], permission[1])
...
def test11():
    start = False
    with open('jni/jni.json') as file_obj:
...
def test13():
    ...
    with open('jni/jni.json') as file_obj:
        ...
        for cpp_jni in cpp_jni_list:
            ...
            # android.permission.RECORD_AUDIO [处理调用函数没到最低端，暂时写死替代]❓【1】一起解决
            ...
            for var in vars:
                for pair in var:
                    # if 'android_media_MediaPlayer_setDataSourceAndHeaders' in pair:
                        entry_funs.append(pair[-1] + '(')
            print(entry_funs)
            c_cpp_list = find_command_star_node(cpp.replace('/Volumes/android/android-8.0.0_r34/', ''))
            c_cpp_list = c_cpp_list[0]
