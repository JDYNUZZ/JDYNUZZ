...
clang_lib_path = '/hci/chaoran_data/android-7.1.2_r33/out/host/linux-x86/lib64/libclang.so'
...
init_arg_config = '-isystem/hci/chaoran_data/android-7.1.2_r33/prebuilts/clang/host/linux-x86/clang-2690385/lib64/clang/3.8/include'
# Config.set_library_file("/Volumes/android/android-8.0.0_r34/prebuilts/clang/host/darwin-x86/clang-4053586/lib64/libclang.dylib")
h_list = None
...
class file_analyser():
    ...
    def load_cfg(self, index, compiler, file, ninja_args):
        ...
        ast_path = 'ast7.1/' + file.replace('/','_') + '.ast'
        tu = None
    ...
    def run(self, file_str, pro_path,  ninja_args, entry_funs=None, IS_AOSP=True, extend_analyze=True, show_loc=False, print_all_node=False, generate_html=False, anti_search=False, only_preprocess = False):
        ...
        if extend_analyze:
            ...
            for tem in not_sys_include_paths:
                ...
                # print("*.h:", tem)
                if IS_AOSP:
                    c_cpp_list = find_command(tem, version_prefix='7.1', compdb=True, project_path=project_path)
...
def test11():
    start = False
    with open('jni7.1/jni.json') as file_obj:
        ...
        for cpp_jni in cpp_jni_list:
            ...
            print(entry_funs)
            c_cpp_list = find_command_star_node(cpp.replace(project_path, ''), version_prefix='7.1', compdb=True)
            c_cpp_list = c_cpp_list[0]
