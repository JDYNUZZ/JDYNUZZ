...
clang_prepath = 'prebuilts/clang/host/linux-x86/clang-r383902b1/'
clang_lib_path = project_path + clang_prepath + 'lib64/libc++.so'
# clang_lib_path = '/hci/chaoran_data/android-11.0.0_r39/out/host/linux-x86/lib64/libclang.so'
# clang_lib_path = '/hci/chaoran_data/pythonProject/clang_api24/libclang.so'
# print(clang_lib_path)
Config.set_library_file('/hci/chaoran_data/android-11.0.0_r39/prebuilts/clang/host/linux-x86/clang-r383902b1/lib64/libclang.so.11git')
# Config.set_library_file(clang_lib_path)
init_arg_config = '-isystem/hci/chaoran_data/android-11.0.0_r39/prebuilts/clang/host/linux-x86/clang-r383902b1/lib64/clang/11.0.2/include'
h_list = None
...
class file_analyser():
    ...
    def service_str_trans(self, str):
        ...
        if str =='sensorservice':
            ...
            # SurfaceFlinger.h
            self.extend_h_analysis('frameworks/SurfaceFlinger_hwc1.cpp', '11.0', True, project_path, fuzzy=True)
    ...
    def _get_fun_con(self, cursor, debug=True):
        ...
        if tem_head.endswith('.h'):
            ...
            c_cpp_list = find_command(tem_head, version_prefix='11.0', compdb=True, project_path=project_path)
    ...
    def show_info(self, node, cur_fun=None, depth=0, print_node=False, if_stmt=None, last_node=None, case_identifier=[''], case_mode=[False], case_level=[10e10]):
        ...
        if node.location.file and (self.print_all_node or print_node) and self.pro_path in node.location.file.name:
            print('\n1358|%2d' % depth + ' ' * depth, node.kind, node.spelling, '|current case_identifier:', case_identifier[0], end='')
                ...
                if(f.location.file is not None and self.pro_path in f.location.file.name):
                    ...
                    if len(r_final) > 0 and self.has_no_ignore_fun(self.fully_qualified_pretty(last_node)) and self.has_no_ignore_fun(self.fully_qualified_pretty(f)):
                        ...
                        if 'AudioPolicyService::createAudioPatch' in method_full:
                            ...
                            self.extend_h_analysis(h_no_I, '11.0', True, project_path)
                            print('.line 1903')
                            # exit(0)
                        ...
                        if self.has_ignore_fun_Ixx(method_full) and len(tema)>0:
                            ...
                            if 'ISurfaceComposer::captureScreen' in self.fully_qualified_pretty(f) and return_class !='no_caller':
                                ...
                                self.extend_h_analysis(f.location.file.name, '11.0', True, project_path, fuzzy=True)
                                # exit(8)
    ...
    def load_cfg_normal(self, index, file, args):
        ...
        syspath = ccsyspath.system_include_paths(
            '/hci/chaoran_data/android-11.0.0_r39/prebuilts/clang/host/linux-x86/clang-r383902b1/bin/clang++')
    ...
    def load_cfg(self, index, compiler, file, ninja_args):
        ...
        # ninja_args.append('-I/hci/chaoran_data/android-11.0.0_r39/prebuilts/clang/host/linux-x86/clang-r383902b1/lib64/clang/11.0.2/share')
        # ninja_args.append('-isystem')
        # ninja_args.append('/hci/chaoran_data/android-11.0.0_r39/prebuilts/clang/host/linux-x86/clang-r383902b1')


        ast_path = 'ast11.0/' + file.replace('/', '_') + '.ast'
        tu = None
        ...
        print('=======python Clang END========')
        # args = ninja_args

        # command = '/hci/chaoran_data/android-11.0.0_r39/prebuilts/clang/host/linux-x86/clang-r383902b1/bin/clang++ -v '
        # for i in range(1, len(ninja_args)):
        ...
        for d in tu.diagnostics:
            if d.severity == d.Error or d.severity == d.Fatal:
                ...
                if 'use of undeclared identifier' in str(d) or 'file not found' in str(d):
                    return None
                # print('===========')
                # print('prebuilts/clang/host/linux-x86/clang-r383902b1/bin/clang', end=' ')
                # for temmm in ninja_args:
                #     print(temmm, end=' ')
                # print('===========')
                raise Exception('aaaaaaaaaaaaaaaaaa')
            else:
        ...
        if extend_analyze:
            ...
            for tem in not_sys_include_paths:
                ...
                if IS_AOSP:
                    ...
                    c_cpp_list = find_command(tem, version_prefix='11.0', compdb=True, project_path=project_path)
                    # for i_c_cpp_lists, c_cpp_list in enumerate(c_cpp_lists):
                    #     print('.line 2293 ', i_c_cpp_lists, '/', len(c_cpp_lists))
                    if c_cpp_list is not None and project_path + c_cpp_list['file'] not in self.analyzed_cpp:
                        # next_file = '/Volumes/android/android-7.0.0_r33/' + c_cpp_list['source']
...
def test11():
    start = False
    with open('jni11.0/jni.json') as file_obj:
        ...
        for cpp_jni in cpp_jni_list:
            ...
            print(entry_funs)
            c_cpp_list = find_command_star_node(cpp.replace(project_path, ''), '11.0', compdb=True)
            c_cpp_list = c_cpp_list[0]
...
def test13():
    ...
    with open('jni11.0/jni.json') as file_obj:
        ...
        for cpp_jni in cpp_jni_list:
            ...
            '''
            ****** find_var_local ******
            find var: instance | at android::Singleton::getInstance() <SourceLocation file '/hci/chaoran_data/android-11.0.0_r39/system/core/libutils/include/utils/Singleton.h', line 51, column 18>
            '''
